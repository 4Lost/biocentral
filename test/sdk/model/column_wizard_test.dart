  ...
  group('Column Wizard', () {
    ...
    test('Column Wizard detects column type correctly', () async {
      ...
      ColumnWizard columnWizard =
          await wizardRepo.getColumnWizardForColumn(columnNames: [columnName], valueMap: strings.asStringMap());
      expect(columnWizard is StringColumnWizard, equals(true));

      final List<int> ints = [1234, 555, -31904, 0];
      columnWizard = await wizardRepo.getColumnWizardForColumn(columnNames: [columnName], valueMap: ints.asStringMap());
      expect(columnWizard is IntColumnWizard, equals(true));

      final List<double> doubles = [125.0, -313.1232, -950.1];
      columnWizard = await wizardRepo.getColumnWizardForColumn(columnNames: [columnName], valueMap: doubles.asStringMap());
      expect(columnWizard is DoubleColumnWizard, equals(true));

      final List<dynamic> mixed = ['Hallo', 1234, 901.23];
      columnWizard = await wizardRepo.getColumnWizardForColumn(columnNames: [columnName], valueMap: mixed.asStringMap());
      expect(columnWizard is StringColumnWizard, equals(true));
    });
    test('Column Wizard calculates metrics correctly', () async {
      ...
      ColumnWizard columnWizard =
          await wizardRepo.getColumnWizardForColumn(columnNames: [columnName], valueMap: ints.asStringMap());
